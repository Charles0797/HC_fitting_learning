...
def cyclic(protocol, params):
    # 位移协议平滑处理
    protocol = savgol_filter(protocol, window_length=5, polyorder=2)
    # 建立一维单自由度模型
...
if __name__ == "__main__":
    ...
    protocol = df["displacement"].values
    target_curve = df["force"].values

    # Steel02材料参数 (需包含更多参数)
    # 正确参数设置示例 (参数值需在合理区间)
    params = [
        125.0,  # fy (屈服强度, MPa)
        11500,  # E (弹性模量, MPa 需确保与钢材匹配)
        0.15,  # b (应变硬化比，建议0.005~0.03)
    ...
    error_norm = np.linalg.norm(target_curve - forces_single, ord=2)
    print("curve_error = ", error_norm)
    ...
    plt.plot(protocol, target_curve, linestyle='--', color= 'r')

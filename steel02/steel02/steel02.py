...
class HysteresisEnv:
    ...
    def __init__(self):
        # 两个可调参数的取值范围
        self.param_ranges = [[1, 1000], [1, 50000]]
        # 从 Excel 文件中读取数据
        df = pd.read_excel("测试2舍去破坏阶段.xlsx")
        self.protocol = df["displacement"].values
        self.protocol = savgol_filter(self.protocol, window_length=5, polyorder=2)
        self.target_curve = df["force"].values

        self.target_range = np.max(self.target_curve) - np.min(self.target_curve)
        self.scale_factors = [10, 500]
        self.reset()
    ...
    def hysteretic_curve(self, params):
        ...
        ops.uniaxialMaterial("Steel02", 1, params[0], params[1], 0.15, 15.0, 0.925, 0.15, 0.0, 1.0, 0.0, 1.0)
    ...
    def step(self, action):
        ...
        """
        # scale_factors = [50, 200]  # 可根据需要设置更新尺度
        new_params = []
        done = False
        ...
        # # 调试打印，可以查看误差是否在合理范围内
        # print(
        #     f"curve_error = {error_norm:.4f}, reward = {reward:.4f}")
...
def train_ddpg(env, actor, critic, actor_optimizer, critic_optimizer,
    ...
    target_protocol = env.protocol
    protocol_min = np.min(target_protocol)
    protocol_max = np.max(target_protocol)
    target_min = np.min(target_curve)
    target_max = np.max(target_curve)
    fixed_xlim = (protocol_min * 1.2, protocol_max * 1.2)
    fixed_ylim = (target_min * 1.2, target_max * 1.2)
    ...
    for episode in range(num_episodes):
        ...
        if prev_best_reward > 9:
            ...
            factor = 1
        env.scale_factors = [10 / factor, 500 / factor]
...
if __name__ == "__main__":
    ...
    print("Target parameters: 125, 11500, 0.15")
